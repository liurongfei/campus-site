<template>
  <div class="pt-3">
    <div class="mx-3" v-for="friend in friends" :key="friend.id">
      <div class="d-flex">
        <router-link :to="{path:'/personalDetail',query:{id:friend.userId,returnPath:'/follows'}}" id="image" :style="{backgroundImage: 'url(http://localhost/' + friend.userImage + ')'}" class="bg-success text-center pt-3"></router-link>
...
export default {
  ...
  activated(){
    ...
    this.$axios.post('http://localhost/getFollows',this.$qs.stringify(data))
    .then((res)=>{
